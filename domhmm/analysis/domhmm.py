...
class PropertyCalculation(LeafletAnalysisBase):
    ...
    def process_clustering_task(self, args):
        i, j, leaflet_ = args
        ...
        return j, frame_number, clusters
    ...
    def result_clustering(self):
        ...
        tasks = [(i, j, leaflet_) for i in range(self.n_frames) for j, leaflet_ in enumerate(['upper', 'lower'])]
        num_workers = multiprocessing.cpu_count()
        chunksize = max(1, len(tasks) // (num_workers * 4))
        with multiprocessing.Pool() as pool:
            results = list(tqdm(pool.imap(self.process_clustering_task, tasks, chunksize=chunksize), total=len(tasks)))

        for j, frame_number, clusters in results:
