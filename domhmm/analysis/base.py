...
class LeafletAnalysisBase(AnalysisBase):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if leaflet_select != None:

            #If the argument leaflet_select is not None, the user specified its own leaflet selection. The code just checks if there are exactly two groups
            # and raises an assertion if not. It then stores the selection in a dictionary

            assert len(leaflet_select) == 2, f"Bilayer is required. {len(leaflet_select)} are found in leaflet_select list."

            #Init empty dict to store AtomGroups
            self.leaflet_selection = {}

            #Iterate over both entries
            for i in range(2): self.leaflet_selection[str(i)] = self.universe.select_atoms(leaflet_select[i])
            ...
            #Init empty dict to store AtomGroups -> That would be not necessary but I want the same variables for the user specified case or the automatic case
            self.leaflet_selection = {}

            #Iterate over found groups
            for idx, leafgroup in enumerate(self.leafletfinder.groups_iter()): self.leaflet_selection[str(idx)] = leafgroup
    ...
    def get_leaflet_resids(self):
        ...
        for idx, leafgroup in zip(self.leaflet_selection.keys(), self.leaflet_selection.values()):
            ...
            for resid in uni_leaf_resids: self.leaflet_resids[idx] += self.universe.select_atoms(f"resid {resid}")
    ...
    def get_leaflet_heads_tails(self):
        ...
        for resname in np.unique( self.memsele.resnames ):
            ...
            #Prepare a MDAnalysis selection string (I.e. ['P', 'O11', 'O12', 'O13', 'O14'] -> 'name P or name O11 or name O12 or name O13 or name O14')
            head_sele_str = 'name ' + (' or name ').join(self.heads[resname])
                
            #Select for correct lipid type (I.e. 'name P or name O11 or name O12 or name O13 or name O14' -> 'resname POPC and (name P or name O11 or name O12 or name O13 or name O14)')
            ...
            for i, tail in enumerate(self.tails[resname]):
                ...
                for j in range( n_pairs ):
                    ...
                    #Select for correct lipid type (I.e. I.e. ['C22', 'H2R'] -> 'name C22 or name H2R' -> 'resname POPC and ('name C22 or name H2R')')
        ...
        for idx, leafgroup in zip(self.leaflet_selection.keys(), self.leaflet_selection.values()):
            ...
            for resid in tqdm(leaf_resids):
                ...
                if idx == "0": self.resid_heads_selection_0[str(resid)] = resid_selection.intersection(head_selection_per_type[resid_resname])
                else: self.resid_heads_selection_1[str(resid)] = resid_selection.intersection(head_selection_per_type[resid_resname])

                #------------------------------------------------------TAIL SELECTION------------------------------------------------------#
                #Make an extra dictionary for each tail
                if idx == "0": self.resid_tails_selection_0[str(resid)] = {}
                else: self.resid_tails_selection_1[str(resid)] = {}
                ...
                for i, tail in enumerate(self.tails[resid_resname]):
                    ...
                    if idx == "0": self.resid_tails_selection_0[str(resid)][str(i)] = []
                    else: self.resid_tails_selection_1[str(resid)][str(i)] = []
                    ...
                    for j in range( n_pairs ):
                        ...
                        if idx == "0": self.resid_tails_selection_0[str(resid)][str(i)].append( resid_selection.intersection(tails_selection_per_type[resid_resname][str(i)][j]) )
                        else: self.resid_tails_selection_1[str(resid)][str(i)].append( resid_selection.intersection(tails_selection_per_type[resid_resname][str(i)][j]) )
